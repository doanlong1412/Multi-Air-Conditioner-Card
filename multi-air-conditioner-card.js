...
const AC_TRANSLATIONS = {
  ...
  de: {
    ...
    edVaneVertical: '↕ Vertikale Lamelle (input_select)',
  ...
  fr: {
    ...
    edVaneVertical: '↕ Volet vertical (input_select)',
  ...
  nl: {
    ...
    edVaneVertical: '↕ Verticale lamel (input_select)',
  ...
  pl: {
    ...
    edVaneVertical: '↕ Pionowa żaluzja (input_select)',
  ...
  sv: {
    ...
    edVaneVertical: '↕ Vertikal riktning (input_select)',
  ...
  hu: {
    ...
    edVaneVertical: '↕ Függőleges lamella (input_select)',
  ...
  cs: {
    ...
    edVaneVertical: '↕ Vertikální lamela (input_select)',
  ...
  it: {
    ...
    edVaneVertical: '↕ Aletta verticale (input_select)',
  ...
  pt: {
    ...
    edVaneVertical: '↕ Aleta vertical (input_select)',
...
.sl-hdr{display:flex;align-items:flex-start;justify-content:space-between;gap:6px}
.sl-title{font-size:13px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:var(--cv-greet-name,rgba(255,255,255,0.9));overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:180px}
.sl-sensor-row{display:flex;gap:6px;align-items:center;flex-wrap:wrap}
.sl-sensor-item{font-size:11px;color:rgba(255,255,255,0.7);font-family:'Orbitron',sans-serif;font-weight:600;white-space:nowrap}
.sl-badge{display:flex;align-items:center;gap:4px;background:rgba(0,20,50,0.32);border:1px solid rgba(255,255,255,0.2);border-radius:20px;padding:3px 8px 3px 5px;flex-shrink:0}
...
.sl-temp-lbl{font-size:8px;letter-spacing:3px;text-transform:uppercase;color:var(--cv-dial-lbl,rgba(255,255,255,0.5));font-weight:500}
.sl-temp-val{font-family:'Orbitron',sans-serif;font-size:40px;font-weight:800;line-height:1;transition:color 0.6s ease}
.sl-temp-feel{font-size:10px;color:var(--cv-comfort,rgba(255,255,255,0.55));margin-top:4px;font-weight:300;text-align:center;max-width:120px;line-height:1.4}
...
class AcControllerCardV2 extends HTMLElement {
  ...
  _renderFull() {
    ...
    // Room temp percentage (for INNER arc r=76)
    ...
    var swingBtn = '<button class="swing-btn" id="btn-swing" style="border-radius:12px;padding:6px 10px;background:rgba(0,20,50,0.25);border:1px solid rgba(255,255,255,0.18);transition:all 0.2s">'
      + swingSvg
      + '<span class="swing-lbl" style="margin-top:2px">' + swingCurrentLabel + '</span>'
      + '</button>';
    ...
    // Mode popup button (single button showing current mode, opens popup on tap)
    var modePopupIcon = (mode.icon && mode.icon.indexOf('mdi:') === 0)
      ? '<ha-icon icon="' + mode.icon + '" style="--mdc-icon-size:18px;color:' + mode.color + ';width:18px;height:18px;display:inline-flex;align-items:center"></ha-icon>'
      : '<span style="font-size:16px">' + mode.icon + '</span>';
    var modePopupBtn = '<button class="mode-btn mode-btn--active" id="btn-mode-popup" data-hvac="' + hvac + '" style="--bc:' + mode.color + ';--bg:' + mode.glow + ';padding:8px 22px;border-radius:20px;min-width:120px">'
      + '<span class="mode-icon">' + modePopupIcon + '</span>'
    ...
    if (isSuperLite) {
      ...
      var slHtml = '<div class="card card--super-lite' + (isDeepNeonSL ? ' card--deep-neon' : '') + '" style="--accent:' + mode.color + ';--glow:' + mode.glow + ';background:' + bgGrad + ';' + _customCssVarsSL + '">'
        ...
        + (cfg.show_greet !== false ? '    <span class="sl-title">' + tr.greet() + ' ' + (cfg.owner_name || tr.cardSub) + '</span>' : '    <span class="sl-title">' + (cfg.owner_name || tr.cardSub) + '</span>')
        ...
        + (slEnvTemp || slEnvHumidity || (slShowRoomPower && slRoomPowerRaw) ? (
            '    <span class="sl-sensor-row">'
          + (slEnvTemp     ? '<span class="sl-sensor-item" style="color:rgba(255,255,255,' + (slEnvIsRoom ? '0.9' : '0.65') + ')">' + (slEnvIsRoom ? '&#127968;' : '&#127777;') + ' ' + slEnvTemp + '</span>' : '')
          + (slEnvHumidity ? '<span class="sl-sensor-item" style="color:rgba(255,255,255,' + (slEnvIsRoom ? '0.75' : '0.55') + ')">&#128167; ' + slEnvHumidity + '</span>' : '')
          + (slShowRoomPower && slRoomPowerRaw ? '<span class="sl-sensor-item">&#9889; ' + slRoomPowerRaw + '</span>' : '')
          + '    </span>'
          ) : '')
        + '  </div>'
...
+ '<div class="mode-grid" style="justify-content:center">' + modePopupBtn + '</div>'
...
+ '</div>'; // end .card
  ...
  _openSelectPopup(anchor, options, current, onSelect) {
    // Glass blur popup for mode/fan/swing selection (Full/Lite)
    var old = document.getElementById('ac-sel-popup-overlay');
    if (old) old.remove();
    var oldPop = document.getElementById('ac-sel-popup');
    if (oldPop) oldPop.remove();

    var overlay = document.createElement('div');
    overlay.id = 'ac-sel-popup-overlay';
    overlay.style.cssText = 'position:fixed;inset:0;z-index:9990;background:rgba(0,0,0,0.25)';
    document.body.appendChild(overlay);

    var pop = document.createElement('div');
    pop.id = 'ac-sel-popup';
    var popStyle = this._config && this._config.popup_style;
    var isWave = popStyle === 'wave';
    pop.style.cssText = [
      'position:fixed','z-index:9999',
    ...
    for (var i = 0; i < options.length; i++) {
      var isActive = options[i] === current;
      var delay = (i * 0.04 + 0.03).toFixed(2) + 's';
      html += '<div class="sel-ri" data-idx="' + i + '" style="'
        + 'display:flex;align-items:center;gap:10px;padding:10px 14px;'
        + 'border-radius:14px;cursor:pointer;font-size:13px;font-weight:600;'
        + 'color:' + (isActive ? '#ffffff' : 'rgba(255,255,255,0.75)') + ';'
        + 'background:' + (isActive ? 'rgba(59,158,255,0.25)' : 'transparent') + ';'
        + 'border:1px solid ' + (isActive ? 'rgba(59,158,255,0.4)' : 'transparent') + ';'
        + 'transition:all 0.15s;animation:slBubblePop 0.35s cubic-bezier(0.22,1,0.36,1) both;'
        + 'animation-delay:' + delay + '">'
        + '<span style="flex:1">' + options[i] + '</span>'
    ...
    pop.querySelectorAll('[data-idx]').forEach(function(el) {
      el.addEventListener('click', function() {
        var idx = parseInt(el.dataset.idx);
        closePop();
        if (onSelect && options[idx] !== undefined) onSelect(options[idx]);
      });
      el.addEventListener('mouseenter', function() { el.style.background = 'rgba(255,255,255,0.08)'; });
      el.addEventListener('mouseleave', function() {
        var isA = options[parseInt(el.dataset.idx)] === current;
        el.style.background = isA ? 'rgba(59,158,255,0.25)' : 'transparent';
      });
    });
  }
  ...
  _bind() {
    ...
    // ── Mode popup (replaces mode-grid buttons) ──────────────────────
    onTap(r.getElementById('btn-mode-popup'), function() {
      var id = ROOMS[self._activeIdx].id;
      var curMode = self._s(id) || 'off';
      var cfg2 = self._config || {};
      var modeKeys2 = ['auto','cool','heat','dry','fan_only'];
      var modeShow2 = { auto:'show_auto', cool:'show_cool', heat:'show_heat', dry:'show_dry', fan_only:'show_fan_only' };
      ...
      for (var mi = 0; mi < modeKeys2.length; mi++) {
        ...
        if (mk2 === 'auto' && cfg2.show_auto !== true) continue;
        if (mk2 !== 'auto' && cfg2[modeShow2[mk2]] === false) continue;
      ...
      self._openSelectPopup(r.getElementById('btn-mode-popup'), labels, curLabel, function(val) {
        for (var ms = 0; ms < items.length; ms++) {
          if (items[ms].label === val) {
            var chosenMode = items[ms].val;
            if (chosenMode && chosenMode !== 'off') self._hvacModeSave(id, chosenMode);
            self._call('climate','set_hvac_mode',{entity_id:id, hvac_mode:chosenMode});
            break;
          }
        }
      });
    ...
    onTap(r.getElementById('btn-fan-cycle'), function() {
      ...
      var lang5 = (self._config && self._config.language) || 'vi';
      var tr5 = AC_TRANSLATIONS[lang5] || AC_TRANSLATIONS.vi;
      var fanLbls5 = tr5.fans || ['Auto','Min','Low','Low-Mid','Medium','High-Mid','High','Max','Low/Auto','High/Auto','Quiet'];
      var popLabels5 = levels.map(function(lv) {
        var fIdx5 = FAN_LEVELS.indexOf(lv);
        return (fIdx5 >= 0 && fIdx5 < fanLbls5.length) ? fanLbls5[fIdx5] : lv;
      });
      var curLabel5 = (function() {
        var ci5 = FAN_LEVELS.indexOf(cur);
        return (ci5 >= 0 && ci5 < fanLbls5.length) ? fanLbls5[ci5] : cur;
      })();
      self._openSelectPopup(r.getElementById('btn-fan-cycle'), popLabels5, curLabel5, function(val) {
        for (var fi5 = 0; fi5 < levels.length; fi5++) {
          if (popLabels5[fi5] === val) {
            self._call('climate','set_fan_mode',{entity_id:id, fan_mode:levels[fi5]});
            break;
          }
        }
      });
    ...
    (function() {
      ...
      var dragTimer = null;
      var lastTemp = null;
      var tUnit = (self._config && self._config.temp_unit) || 'C';
      ...
      function onDragMove(e) {
        ...
        if (setEl) {
          var degSym = tUnit === 'F' ? '\u00b0F' : '\u00b0C';
          var dispTemp = tUnit === 'F' ? Math.round(temp * 9/5 + 32) : temp;
          setEl.textContent = dispTemp + degSym;
        }
    ...
    onTap(r.getElementById('btn-swing'), function() {
      ...
      if (vaneEntity && self._hass && self._hass.states[vaneEntity]) {
        ...
        var vaneOpts = (vaneState.attributes && Array.isArray(vaneState.attributes.options)) ? vaneState.attributes.options : [];
        var vaneLabels = vaneOpts.slice();
        self._openSelectPopup(r.getElementById('btn-swing'), vaneLabels, vaneCur, function(val) {
          self._call('input_select','select_option',{entity_id:vaneEntity, option:val});
        });
      } else {
        ...
        var lang3 = (self._config && self._config.language) || 'vi';
        var tr3 = AC_TRANSLATIONS[lang3] || AC_TRANSLATIONS.vi;
        var swingLabels3 = tr3.swings || ['Fixed','Up/Down','Left/Right','Both'];
        var swingMap3 = { off: swingLabels3[0], vertical: swingLabels3[1], horizontal: swingLabels3[2], both: swingLabels3[3] };
        for (var _sn = 1; _sn <= 6; _sn++) { swingMap3[String(_sn)] = swingLabels3[3 + _sn] || ('Pos ' + _sn); }
        var popLabels = levels.map(function(lv) { return swingMap3[lv] || lv; });
        var curLabel3 = swingMap3[cur] || cur;
        self._openSelectPopup(r.getElementById('btn-swing'), popLabels, curLabel3, function(val) {
          for (var si3 = 0; si3 < levels.length; si3++) {
            if (popLabels[si3] === val) {
              self._call('climate','set_swing_mode',{entity_id:id, swing_mode:levels[si3]});
              break;
            }
          }
        });
  ...
  _bindSuperLite() {
    ...
    // ── Drag-to-adjust temperature on Super Lite dial ──────────────
    (function() {
      ...
      function onDragMove(e) {
        ...
        if (temp === lastTemp) return;
        lastTemp = temp;
        var setEl = r.getElementById('live-set-temp');
        if (setEl) {
          var degSym = tUnit === 'F' ? '\u00b0F' : '\u00b0C';
          var dispTemp = tUnit === 'F' ? Math.round(temp * 9/5 + 32) : temp;
          setEl.textContent = dispTemp + degSym;
      ...
      slDialSvg.addEventListener('mousedown', onDragStart);
      slDialSvg.addEventListener('touchstart', onDragStart, { passive: false });
    })();
    ...
    // Swing popup (Super Lite mini button) — same glass popup style as fan
    var slSwingPopupEl = null;
    function closeSlSwingPopup() {
      if (slSwingPopupEl && slSwingPopupEl.parentNode) { slSwingPopupEl.parentNode.removeChild(slSwingPopupEl); slSwingPopupEl = null; }
      var ov = document.getElementById('sl-swing-overlay-global'); if (ov) ov.parentNode.removeChild(ov);
    }
    function openSlSwingPopup() {
      if (slSwingPopupEl) { closeSlSwingPopup(); return; }
      var lang4   = (self._config && self._config.language) || 'vi';
      var tr4     = AC_TRANSLATIONS[lang4] || AC_TRANSLATIONS.vi;
      var isWave  = (self._config && self._config.popup_style) === 'wave';
      var id      = ROOMS[self._activeIdx].id;
      ...
      if (useVane) {
        var vaneState = self._hass.states[vaneEntity];
        curSwing = vaneState.state || '';
        modes = (vaneState.attributes && Array.isArray(vaneState.attributes.options)) ? vaneState.attributes.options : [];
        swMap = {};
        swIcons = {};
        for (var vi = 0; vi < modes.length; vi++) { swMap[modes[vi]] = modes[vi]; swIcons[modes[vi]] = '↕'; }
      } else {
        curSwing = self._a(id,'swing_mode') || 'off';
        var rawModes = self._a(id,'swing_modes');
        modes = (Array.isArray(rawModes) && rawModes.length > 0) ? rawModes : SWING_LEVELS;
        var swLbls  = tr4.swings || ['Fixed','Up/Down','Left/Right','Both'];
        swMap   = { off: swLbls[0], vertical: swLbls[1], horizontal: swLbls[2], both: swLbls[3] };
        for (var _sn2 = 1; _sn2 <= 6; _sn2++) { swMap[String(_sn2)] = swLbls[3 + _sn2] || ('Pos ' + _sn2); }
        swIcons = { off: '—', vertical: '↕', horizontal: '↔', both: '✖' };
      }

      var overlay = document.createElement('div');
      overlay.id = 'sl-swing-overlay-global';
      overlay.style.cssText = 'position:fixed;inset:0;z-index:9990;background:transparent';
      document.body.appendChild(overlay);
      ...
      var itemsHtml = isWave ? '' : '<div class="sl-pop-shimmer"></div>';
      for (var swi = 0; swi < modes.length; swi++) {
        var swVal   = modes[swi];
        var swLbl   = swMap[swVal] || swVal;
        var swDelay = (swi * 0.04 + 0.03).toFixed(2) + 's';
        var swIcon  = swIcons[swVal] || '↕';
        if (isWave) {
          itemsHtml += '<div class="sl-ri sl-ri-wave' + (curSwing === swVal ? ' active' : '') + '" data-swing-val="' + swVal + '" style="animation-delay:' + swDelay + '">'
            + '<span style="font-size:16px;line-height:1;width:22px;text-align:center">' + swIcon + '</span>'
            + '<span style="flex:1">' + swLbl + '</span>'
            + '<div class="sl-wave-ripple"></div></div>';
        } else {
          itemsHtml += '<div class="sl-ri' + (curSwing === swVal ? ' active' : '') + '" data-swing-val="' + swVal + '" style="animation-delay:' + swDelay + '">'
            + '<span style="font-size:16px;line-height:1;width:22px;text-align:center">' + swIcon + '</span>'
            + '<span style="flex:1">' + swLbl + '</span></div>';
        }
      }
      pop.innerHTML = itemsHtml;

      var swBtn = r.getElementById('sl-btn-swing-sl');
      var swRect = swBtn ? swBtn.getBoundingClientRect() : {left:0,right:140,top:0,bottom:40,width:140};
      ...
      pop.querySelectorAll('[data-swing-val]').forEach(function(item) {
        item.addEventListener('click', function(e) {
          e.stopPropagation();
          if (isWave) _slWaveRipple(item, e);
          var val = item.dataset.swingVal;
          var doCall = function() {
            if (useVane) {
              self._call('input_select','select_option',{entity_id:vaneEntity, option:val});
            } else {
      ...
      overlay.addEventListener('click', closeSlSwingPopup);
    }
    var slSwingBtnEl = r.getElementById('sl-btn-swing-sl');
    if (slSwingBtnEl) slSwingBtnEl.addEventListener('click', function(e) { e.stopPropagation(); openSlSwingPopup(); });
    function onOutsideSlSwingClick(e) {
      if (!slSwingPopupEl) return;
      if (slSwingPopupEl.contains(e.target)) return;
      var sb = r.getElementById('sl-btn-swing-sl'); if (sb && sb.contains(e.target)) return;
      closeSlSwingPopup();
    }
    document.addEventListener('click', onOutsideSlSwingClick);
    var prevSlCleanup2 = self._slCleanup;
    self._slCleanup = function() { document.removeEventListener('click', onOutsideSlSwingClick); closeSlSwingPopup(); if (prevSlCleanup2) prevSlCleanup2(); };
...
class MultiAcCardEditor extends HTMLElement {
  ...
  // ── Tạo ha-entity-picker bằng createElement (không dùng innerHTML) ────────
  _syncPickers() {
    if (!this._hass || !this.shadowRoot) return;
    const self = this;

    function createPicker(slot, domain, value, onChange) {
      // Nếu slot đã có picker rồi, chỉ update hass + value
      var existing = slot.querySelector('ha-entity-picker');
      if (existing) {
        existing.hass = self._hass;
        if (value && existing.value !== value) existing.value = value;
        return;
      }
      // Tạo mới bằng createElement
    ...
    // Global entity pickers (data-ep-key)
    this.shadowRoot.querySelectorAll('.ep-slot[data-ep-key]').forEach(function(slot) {
      var key = slot.dataset.epKey;
      var domain = slot.dataset.epDomain;
      var saved = self._config[key] || '';
      createPicker(slot, domain, saved, function(val) {
        var c = Object.assign({}, self._config);
    ...
    // Per-room pickers
    var ents = (this._config.entities || []).slice();

    // Room climate entity (data-ep-room)
    this.shadowRoot.querySelectorAll('.ep-slot[data-ep-room]').forEach(function(slot) {
      var idx = parseInt(slot.dataset.epRoom);
      var saved = (ents[idx] && ents[idx].entity_id) || '';
      createPicker(slot, 'climate', saved, function(val) {
        var e2 = (self._config.entities || []).slice();
        while (e2.length <= idx) e2.push({});
        if (val) e2[idx] = Object.assign({}, e2[idx], { entity_id: val });
        else delete e2[idx].entity_id;
        self._config = Object.assign({}, self._config, { entities: e2 });
        self._fire();
      });
    });

    // Room temp sensor (data-ep-room-temp)
    this.shadowRoot.querySelectorAll('.ep-slot[data-ep-room-temp]').forEach(function(slot) {
      var idx = parseInt(slot.dataset.epRoomTemp);
      var saved = (ents[idx] && ents[idx].temp_entity) || '';
      createPicker(slot, 'sensor', saved, function(val) {
        var e2 = (self._config.entities || []).slice();
        while (e2.length <= idx) e2.push({});
        if (val) e2[idx] = Object.assign({}, e2[idx], { temp_entity: val });
        else delete e2[idx].temp_entity;
        self._config = Object.assign({}, self._config, { entities: e2 });
        self._fire();
      });
    });

    // Room humidity sensor (data-ep-room-hum)
    this.shadowRoot.querySelectorAll('.ep-slot[data-ep-room-hum]').forEach(function(slot) {
      var idx = parseInt(slot.dataset.epRoomHum);
      var saved = (ents[idx] && ents[idx].humidity_entity) || '';
      createPicker(slot, 'sensor', saved, function(val) {
        var e2 = (self._config.entities || []).slice();
        while (e2.length <= idx) e2.push({});
        if (val) e2[idx] = Object.assign({}, e2[idx], { humidity_entity: val });
        else delete e2[idx].humidity_entity;
        self._config = Object.assign({}, self._config, { entities: e2 });
        self._fire();
      });
    });

    // Room power sensor (data-ep-room-power)
    this.shadowRoot.querySelectorAll('.ep-slot[data-ep-room-power]').forEach(function(slot) {
      var idx = parseInt(slot.dataset.epRoomPower);
      var saved = (ents[idx] && ents[idx].power_entity) || '';
      createPicker(slot, 'sensor', saved, function(val) {
        var e2 = (self._config.entities || []).slice();
        while (e2.length <= idx) e2.push({});
        if (val) e2[idx] = Object.assign({}, e2[idx], { power_entity: val });
        else delete e2[idx].power_entity;
        self._config = Object.assign({}, self._config, { entities: e2 });
        self._fire();
      });
    });

    // Room vane vertical entity (data-ep-room-vane)
    this.shadowRoot.querySelectorAll('.ep-slot[data-ep-room-vane]').forEach(function(slot) {
      var idx = parseInt(slot.dataset.epRoomVane);
      var saved = (ents[idx] && ents[idx].vane_vertical_entity) || '';
      createPicker(slot, 'input_select', saved, function(val) {
        var e2 = (self._config.entities || []).slice();
        while (e2.length <= idx) e2.push({});
        if (val) e2[idx] = Object.assign({}, e2[idx], { vane_vertical_entity: val });
        else delete e2[idx].vane_vertical_entity;
        self._config = Object.assign({}, self._config, { entities: e2 });
        self._fire();
      });
    });
...
<div class="ac-row">
  ...
  <div class="row">
    ...
    <div class="ep-slot" data-ep-room-vane="${i}" data-ep-domain="input_select"></div>
  </div>
  <div class="row">
...
<style>
  ...
  .ep-slot { display:block;width:100%;min-height:40px; }
  .ep-slot ha-entity-picker { display:block;width:100%; }
  /* language */
